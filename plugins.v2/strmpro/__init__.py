...
class StrmPro(_PluginBase):
    ...
    # 插件图标
    plugin_icon = "https://raw.githubusercontent.com/yangchen51920/MoviePilot-Plugins/main/icons/123Pan_A.png"
    # 插件版本
    plugin_version = "1.9"
    # 插件作者
    ...
    def init_plugin(self, config: dict = None):
        # ========== 新增：保存旧配置，用于检测变更后清理strm ==========
        old_strm_dir_conf = self._strm_dir_conf.copy() if self._strm_dir_conf else {}
        old_local_strm_dir_conf = self._local_strm_dir_conf.copy() if self._local_strm_dir_conf else {}
        # ========== 新增结束 ==========
        ...
        if config:
            ...
            # ========== 新增：检测配置变更，清理已移除/变更映射的旧strm文件 ==========
            if old_strm_dir_conf or old_local_strm_dir_conf:
                self.__cleanup_removed_configs(old_strm_dir_conf, old_local_strm_dir_conf)
            # ========== 新增结束 ==========
            ...
            if self._enabled:
                ...
                # 启动目录监控
                if self._monitor or self._local_strm_monitor:
                    for mon_path in self._strm_dir_conf.keys():
                        is_local = self._local_strm_enabled and mon_path in self._local_strm_dir_conf

                        # 云盘路径需要全局monitor开关
                        if not is_local and not self._monitor:
                            continue
                        # 本地路径需要local_strm_monitor开关
                        if is_local and not self._local_strm_monitor:
                            continue
                        ...
                        if Path(mon_path).exists():
                            ...
                            logger.info(f"已启动目录监控: {mon_path} {'(本地路径模式)' if is_local else ''}")
                        else:
    ...
    def __cleanup_removed_configs(self, old_strm_dir_conf: dict, old_local_strm_dir_conf: dict):
        """
        检测配置变更，清理已移除或源路径变更的映射对应的旧strm文件

        两种场景：
        1. 配置行被删除 → 对应的strm输出目录整个清理
        2. 源路径变更（strm输出路径不变）→ 清理旧源路径对应的所有strm文件
        """
        # 合并所有旧映射：mon_path → strm_dir
        all_old_mappings = {}
        all_old_mappings.update(old_strm_dir_conf)

        # 收集当前所有 strm_dir 用于判断是否被其他映射复用
        current_strm_dirs = set(self._strm_dir_conf.values())

        for old_mon_path, old_strm_dir in all_old_mappings.items():
            if old_mon_path not in self._strm_dir_conf:
                # 该映射已被移除
                if old_strm_dir not in current_strm_dirs:
                    # strm_dir 没有被其他映射复用 → 整个清理
                    logger.info(f"配置已移除，清理strm目录: 源={old_mon_path} → strm目录={old_strm_dir}")
                    self.__cleanup_strm_dir(old_strm_dir)
                else:
                    # strm_dir 被其他映射复用了（源路径变更）
                    # 清理该目录下所有旧strm文件，新源路径扫描时会重新生成
    ...
    def __cleanup_strm_dir(self, strm_dir: str):
        ...
        try:
            ...
            # 递归删除所有关联文件（nfo、jpg、png、json、ass、srt等）
